...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Changed query to use tenant table instead of owner
        $stmt = $conn->prepare("SELECT tenant_id, owner_id FROM tenant WHERE username = ? AND password = ?");
        if (!$stmt) {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            // Store both tenant_id and owner_id in session
